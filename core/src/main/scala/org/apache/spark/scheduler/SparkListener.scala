...
@DeveloperApi
case class SparkListenerExecutorFiberUpdate(
    execId: String,
    fiberUpdateInfo: String,
...
trait SparkListener {
  ...
  /**
   * Called when the driver receives fiber update info.
   */
  def onFiberUpdate(fiberUpdate: SparkListenerExecutorFiberUpdate) {}
}
