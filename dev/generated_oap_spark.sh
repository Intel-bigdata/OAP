...
function usage(){
  printf "./generated_oap_spark.sh [Your Spark Source Root Dir] [OAP Source Root Dir]"
}

function move_single_file (){
  rp=`echo "$1" | awk '{split($0, a, "main"); print a[2]}'`
  fp="$2$rp"
  mkdir -p "`dirname $fp`"
  cp $1 $fp
  echo "Move $1 to $fp \n"
...
function move_files_from_list_to_oap(){
  ...
  do
    ...
    file_path=`find $SPARK_SOURCE_DIR -name $line`
...
OAP_SPARK_SOURCE_DIR="${OAP_SOURCE_DIR}/oap-spark/src/main/"
