...
class SpinachReadSupportImpl extends SpinachReadSupport[UnsafeRow] with Logging {
  ...
  override def init(context: InitContext): SpinachReadContext = {
    catalystRequestedSchema = {
      ...
      val schemaString = conf.get(SpinachReadSupportImpl.SPARK_ROW_REQUESTED_SCHEMA)
    ...
    catalystReadFromFileSchema = {
      ...
      if (readFromFileSchemaString == null) {
        StructType.fromString(conf.get(SpinachReadSupportImpl.SPARK_ROW_REQUESTED_SCHEMA))
      } else {
    ...
    val spinachReadFromFileSchema =
      SpinachReadSupportImpl.clipParquetSchema(context.getFileSchema, catalystReadFromFileSchema)
  ...
  override def prepareForRead(
    ...
    new SpinachRecordMaterializer(
      parquetRequestedSchema,
      SpinachReadSupportImpl.expandUDT(catalystRequestedSchema))
  }
}
...
object SpinachReadSupportImpl {
  ...
  private def clipParquetType(parquetType: Type, catalystType: DataType): Type = {
    catalystType match {
      ...
      case t: MapType if !isPrimitiveCatalystType(t.keyType) ||
