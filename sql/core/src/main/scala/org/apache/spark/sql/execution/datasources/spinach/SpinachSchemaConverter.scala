...
import org.apache.spark.sql.execution.datasources.spinach.SpinachSchemaConverter.maxPrecisionForBytes
...
class SpinachSchemaConverter(
  ...
  private def convertPrimitiveField(field: PrimitiveType): DataType = {
    ...
    def makeDecimalType(maxPrecision: Int = -1): DecimalType = {
      ...
      SpinachSchemaConverter.checkConversionRequirement(
  ...
  private def convertField(field: StructField, repetition: Type.Repetition): Type = {
    SpinachSchemaConverter.checkFieldName(field.name)
    ...
    field.dataType match {
      ...
      case DecimalType.Fixed(precision, scale) if writeLegacyParquetFormat =>
        Types
          ...
          .precision(precision)
          .scale(scale)
          .length(SpinachSchemaConverter.minBytesForPrecision(precision))
          .named(field.name)
