...
private object NoopUpdater extends ParentContainerUpdater

trait HasParentContainerUpdater {
  def updater: ParentContainerUpdater
}

abstract class SpinachGroupConverter(val updater: ParentContainerUpdater)
  extends GroupConverter with HasParentContainerUpdater

class SpinachPrimitiveConverter(val updater: ParentContainerUpdater)
...
class SpinachRowConverter(
  parquetType: GroupType,
  catalystType: StructType,
  updater: ParentContainerUpdater)
    extends SpinachGroupConverter(updater) with Logging {
  ...
  private def newConverter(
    ...
    catalystType match {
      ...
      case t: ArrayType =>
        new SpinachArrayConverter(parquetType.asGroupType(), t, updater)

      case t: MapType =>
        new SpinachMapConverter(parquetType.asGroupType(), t, updater)

      case t: StructType =>
        new SpinachRowConverter(parquetType.asGroupType(), t, new ParentContainerUpdater {
  ...
  private abstract class ParquetDecimalConverter(
    ...
    protected def decimalFromBinary(value: Binary): Decimal = {
      if (precision <= Decimal.MAX_LONG_DIGITS) {
        ...
        val unscaled = SpinachRowConverter.binaryToUnscaledLong(value)
   ...
   */
  private final class SpinachArrayConverter(
    parquetSchema: GroupType,
    catalystSchema: ArrayType,
    updater: ParentContainerUpdater)
      extends SpinachGroupConverter(updater) {
  ...
  private final class SpinachMapConverter(
    parquetType: GroupType,
...
private object SpinachRowConverter {
