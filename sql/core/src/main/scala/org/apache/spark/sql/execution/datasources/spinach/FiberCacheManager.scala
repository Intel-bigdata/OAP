...
import org.apache.spark.sql.catalyst.expressions.{BaseGenericInternalRow, GenericInternalRow, MutableRow}
  ...
  @transient private val cache =
    CacheBuilder
      ...
      override def load(key: Fiber): FiberByteData = {
        val is: FSDataInputStream = FiberDataFileHandler(key.file)
        key.file.getFiber(is, key.rowGroupId, key.columnIndex)
      }
    })
...
class BatchRow {
  ...
  def reset(rowCount: Int, values: Array[ColumnValues]): BatchRow = {
