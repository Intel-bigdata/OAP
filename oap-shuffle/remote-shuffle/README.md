...
Build this module using the following command in oap-shuffle/remote-shuffle folder or download the pre-built ` oap-remote-shuffle-<version>-with-spark-<version>.jar ` from  [oap-0.9-bin-spark-3.0.0.tar.gz](https://github.com/Intel-bigdata/OAP/releases/download/v0.8.2-spark-3.0.0/oap-0.9-bin-spark-3.0.0.tar.gz) This file needs to be deployed on every compute node that runs Spark. Manually place it on all nodes or let resource manager do the work.
