...
SQL Index and Data Source Cache on Spark 2.4.4 requires a working Hadoop cluster with YARN and Spark. Running Spark on YARN requires a binary distribution of Spark, which is built with YARN support.
...
Start Plasma service on each node with following command, then run your workload. If you install OAP by Conda, you can find `plasma-store-server` under  **$HOME/miniconda2/envs/oapenv/bin/**.
...
- [Column Vector Cache](./Advanced-Configuration.md#Column-Vector-Cache) 
  ...
  This document above uses **binary** cache as example, if your cluster memory resources is abundant enough, you can choose ColumnVector data cache instead of binary cache to spare computation time.
