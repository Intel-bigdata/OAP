...
private[oap] object IndexUtils {
  ...
  /**
   * Get the index file from the data file
   *
  ...
  /**
   * If the OapConf.OAP_INDEX_DIRECTORY is "", get the index path from the path of data file;
   * otherwise from OapConf.OAP_INDEX_DIRECTORY + data file path
   *
  ...
  /**
   * Generated the indexPath based on OapConf.OAP_INDEX_DIRECTORY and the dataPath.
   * if the OapConf.OAP_INDEX_DIRECTORY is "", the indexPath is the dataPath;
   * otherwise the indexPath is OapConf.OAP_INDEX_DIRECTORY + dataPath
   *
   * @param conf     the configuration to get the value of OapConf.OAP_INDEX_DIRECTORY
   * @param dataPath the path of the data file
   * @return the generated index path
   */
  def getIndexPathFromDataPath(conf: RuntimeConfig, dataPath: Path): Path = {
    val indexDirectory = conf.get(OapConf.OAP_INDEX_DIRECTORY.key)
    if (indexDirectory != "") {
      new Path (
  ...
  /**
   * Get the index file path from the data file path
   *
   ...
   * @param indexFile   the index file name
   * @return the index path
   */
