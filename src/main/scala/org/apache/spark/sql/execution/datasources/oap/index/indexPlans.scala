...
case class CreateIndexCommand(
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    // get the dataPath from the FileIndex, here the dataPath does not contain the partition path
    val dataPath = OapUtils.getOutPutPath(fileCatalog)
    // The outputPath is generated by spark.sql.oap.index.directory and the dataPath
    val outPutPath = IndexUtils.getIndexPathFromDataPath(sparkSession.conf, dataPath)
