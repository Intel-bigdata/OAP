...
import org.apache.spark.sql.catalyst.expressions.{Ascending, BoundReference, Descending, FromUnsafeProjection, SortOrder, UnsafeProjection}
...
private[index] case class OapIndexFileWriter(
    configuration: Configuration,
    keySchema: StructType,
    file: Path) {

  def flushToFile(multiHashMap: ArrayListMultimap[InternalRow, Long]): Unit = {
  ...
  private val out = file.getFileSystem(configuration).create(file, true)

  def write(multiHashMap: ArrayListMultimap[InternalRow, Long]): Unit = {
    out.write(s"test ${multiHashMap.size()}".getBytes(Charset.forName("ASCII")))
  }

  // def write(): Unit = out.write()
