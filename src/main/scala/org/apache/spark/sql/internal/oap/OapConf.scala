...
object OapConf {
  ...
  def buildConf(key: String): ConfigBuilder = ConfigBuilder(key).onCreate(SQLConf.register)

  val OAP_PARQUET_ENABLED =
    buildConf("spark.sql.oap.parquet.enable")
      .internal()
  ...
  val OAP_FULL_SCAN_THRESHOLD =
    buildConf("spark.sql.oap.statistics.fullScanThreshold")
      .internal()
  ...
  val OAP_STATISTICS_TYPES =
    buildConf("spark.sql.oap.statistics.type")
      .internal()
  ...
  val OAP_STATISTICS_PART_NUM =
    buildConf("spark.sql.oap.statistics.partNum")
      .internal()
  ...
  val OAP_STATISTICS_SAMPLE_RATE =
    buildConf("spark.sql.oap.statistics.sampleRate")
      .internal()
  ...
  val OAP_STATISTICS_SAMPLE_MIN_SIZE =
    buildConf("spark.sql.oap.statistics.sampleMinSize")
      .internal()
  ...
  val OAP_BLOOMFILTER_MAXBITS =
    buildConf("spark.sql.oap.statistics.bloom.maxBits")
      .internal()
  ...
  val OAP_FIBERCACHE_SIZE =
    buildConf("spark.sql.oap.fiberCache.size")
      .internal()
  ...
  val OAP_FIBERCACHE_STATS =
    buildConf("spark.sql.oap.fiberCache.stats")
      .internal()
  ...
  val OAP_FIBERCACHE_USE_OFFHEAP_RATIO =
    buildConf("spark.sql.oap.fiberCache.use.offheap.ratio")
      .internal()
      .doc("Define the ratio of fiber cache use 'spark.memory.offHeap.size' ratio.")
      .doubleConf
      .createWithDefault(0.7)

  val OAP_COMPRESSION = buildConf("spark.sql.oap.compression.codec")
    .internal()
  ...
  val OAP_INDEX_BTREE_COMPRESSION = buildConf("spark.sql.oap.index.compression.codec")
    .internal()
    .doc("Sets the compression codec use when writing Parquet files. Acceptable values include: " +
        "uncompressed, snappy, gzip, lzo.")
    .stringConf
  ...
  val OAP_ROW_GROUP_SIZE =
    buildConf("spark.sql.oap.rowgroup.size")
      .internal()
  ...
  val OAP_ENABLE_OINDEX =
    buildConf("spark.sql.oap.oindex.enabled")
      .internal()
  ...
  val OAP_ENABLE_EXECUTOR_INDEX_SELECTION =
    buildConf("spark.sql.oap.oindex.eis.enabled")
      .internal()
  ...
  val OAP_EXECUTOR_INDEX_SELECTION_FILE_POLICY =
    buildConf("spark.sql.oap.oindex.file.policy")
      .internal()
  ...
  val OAP_EXECUTOR_INDEX_SELECTION_STATISTICS_POLICY =
    buildConf("spark.sql.oap.oindex.statistics.policy")
      .internal()
  ...
  val OAP_ENABLE_OPTIMIZATION_STRATEGIES =
    buildConf("spark.sql.oap.strategies.enabled")
      .internal()
  ...
  val OAP_INDEX_FILE_SIZE_MAX_RATIO =
    buildConf("spark.sql.oap.oindex.size.ratio")
      .internal()
      .doc("To indicate if enable/disable index cbo which helps to choose a fast query path")
      .doubleConf
      .createWithDefault(0.7)
  ...
  val OAP_INDEXER_CHOICE_MAX_SIZE =
    buildConf("spark.sql.oap.indexer.max.use.size")
      .internal()
  ...
  val OAP_BTREE_ROW_LIST_PART_SIZE =
    buildConf("spark.sql.oap.btree.rowList.part.size")
      .internal()
  ...
  val OAP_INDEX_DISABLE_LIST =
    buildConf("spark.sql.oap.oindex.disable.list")
    .internal()
  ...
  val OAP_HEARTBEAT_INTERVAL =
    buildConf("spark.sql.oap.heartbeatInterval")
    .internal()
  ...
  val OAP_UPDATE_FIBER_CACHE_METRICS_INTERVAL_SEC =
    buildConf("spark.sql.oap.update.fiber.cache.metrics.interval.sec")
      .internal()
  ...
  val OAP_INDEX_BTREE_WRITER_VERSION =
    buildConf("spark.sql.oap.index.btree.writer.version")
      .internal()
  ...
  val OAP_PARQUET_DATA_CACHE_ENABLED =
    buildConf("spark.sql.oap.parquet.data.cache.enable")
      .internal()
