...
class FilterSuite extends QueryTest with SharedSQLContext with BeforeAndAfterEach {
  ...
  override def beforeEach(): Unit = {
    ...
    sql(s"""CREATE TEMPORARY VIEW spinach_test_rowgroup (a INT, b STRING)
           | USING spn
  ...
  override def afterEach(): Unit = {
    ...
    sqlContext.dropTempTable("spinach_test_rowgroup")
  ...
  test("test spinach row group size change") {
    val previousRowGroupSize = sqlContext.conf.getConfString(SQLConf.SPINACH_ROW_GROUP_SIZE.key)
    // change default row group size
    sqlContext.conf.setConfString(SQLConf.SPINACH_ROW_GROUP_SIZE.key, "1025")
    val data: Seq[(Int, String)] = (1 to 3000).map { i => (i, s"this is test $i") }
    data.toDF("key", "value").createOrReplaceTempView("t")
    checkAnswer(sql("SELECT * FROM spinach_test"), Seq.empty[Row])
    sql("insert overwrite table spinach_test select * from t")
    ...
    if (previousRowGroupSize == null) {
      sqlContext.conf.setConfString(SQLConf.SPINACH_ROW_GROUP_SIZE.key,
        SQLConf.SPINACH_ROW_GROUP_SIZE.defaultValueString)
    } else {
      sqlContext.conf.setConfString(SQLConf.SPINACH_ROW_GROUP_SIZE.key,
  ...
  test("test spinach row group size and compression code change through table option") {
    // TODO: How to confirm the config takes effect?
    val data: Seq[(Int, String)] = (1 to 3000).map { i => (i, s"this is test $i") }
    data.toDF("key", "value").createOrReplaceTempView("t")
    checkAnswer(sql("SELECT * FROM spinach_test_rowgroup"), Seq.empty[Row])
    sql("insert overwrite table spinach_test_rowgroup select * from t")
    checkAnswer(sql("SELECT * FROM spinach_test_rowgroup"), data.map { row => Row(row._1, row._2) })
  }
  ...
  test("filtering") {
    ...
    checkAnswer(sql("SELECT * FROM spinach_test WHERE a >= 300"),
