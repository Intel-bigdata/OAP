...
class FiberSensorSuite extends SparkFunSuite with AbstractFiberSensor with Logging {
  ...
  test("test get hosts from FiberSensor") {
    ...
    assert(this.getHosts(filePath) contains (FiberSensor.SPINACH_CACHE_HOST_PREFIX + host1 +
    ...
    assert(this.getHosts(filePath) contains  (FiberSensor.SPINACH_CACHE_HOST_PREFIX + host2 +
