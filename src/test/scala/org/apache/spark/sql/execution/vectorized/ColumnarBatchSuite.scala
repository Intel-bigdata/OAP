...
class ColumnarBatchSuite extends SparkFunSuite {
  ...
  test("ColumnarBatch basic") {
    (MemoryMode.ON_HEAP :: MemoryMode.OFF_HEAP :: Nil).foreach { memMode => {
      ...
      assert(!batch.column(0).isNullAt(0))
      assert(batch.column(1).getDouble(0) == 1.1)
      assert(!batch.column(1).isNullAt(0))
      assert(batch.column(2).isNullAt(0))
  ...
  test("Random flat schema") {
    testRandomRows(flatSchema = true, 15)
  }

  test("Random nested schema") {
    testRandomRows(flatSchema = false, 30)
  }
