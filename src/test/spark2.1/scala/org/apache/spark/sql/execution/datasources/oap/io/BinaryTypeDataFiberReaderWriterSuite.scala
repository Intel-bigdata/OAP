...
class BinaryTypeDataFiberReaderWriterSuite extends DataFiberReaderWriterSuite {
  ...
  protected val dictionary: Dictionary = BinaryDictionary(Array(
    Binary.fromString("oap"),
    Binary.fromString("parquet"),
    Binary.fromString("orc")))

  test("no dic no nulls") {
    // write data
    val column = ColumnVector.allocate(total, BinaryType, MemoryMode.ON_HEAP)
    ...
    // read use batch api
    val ret1 = ColumnVector.allocate(num, BinaryType, MemoryMode.ON_HEAP)
      .asInstanceOf[OnHeapColumnVector]
    ...
    val ret2 = ColumnVector.allocate(num, BinaryType, MemoryMode.ON_HEAP)
      .asInstanceOf[OnHeapColumnVector]
  ...
  test("with dic no nulls") {
    ...
    (0 until num).foreach(i => {
      ...
      assert(ret1.getBinary(i).sameElements(dictionary.decodeToBinary(idx).getBytes))
    })
    ...
    ints.indices.foreach(i => {
      ...
      assert(ret2.getBinary(i).sameElements(dictionary.decodeToBinary(idx).getBytes))
    })
  }
