...
class DoubleTypeDataFiberReaderWriterSuite extends DataFiberReaderWriterSuite {
  ...
  protected val dictionary: Dictionary = DoubleDictionary(Array(0L, 1L, 2L))

  test("no dic no nulls") {
    // write data
    val column = ColumnVector.allocate(total, DoubleType, MemoryMode.ON_HEAP)
    ...
    // read use batch api
    val ret1 = ColumnVector.allocate(num, DoubleType, MemoryMode.ON_HEAP)
      .asInstanceOf[OnHeapColumnVector]
    reader.readBatch(start, num, ret1)
    (0 until num).foreach(i => assert(ret1.getDouble(i) == (i + start).toDouble))

    // read use random access api
    val ret2 = ColumnVector.allocate(num, DoubleType, MemoryMode.ON_HEAP)
      .asInstanceOf[OnHeapColumnVector]
